...
int deepstream_func1()
{
  ...
  GstElement *pipeline = NULL, *source = NULL, *h264parser = NULL,
             ...
             *nvosd = NULL,
             ...
             //新增
             *nvvidconv1 = NULL, *nvvidconv2 = NULL ,*cap_filter = NULL,*cap_filter1 = NULL,
             *convert = NULL, *encoder=NULL,*muxer = NULL;
             //新增
  ...
  sink = gst_element_factory_make ("rtmpsink", "rtmp-sink");
  g_object_set(sink, "location", RTMP_SERVER_URL, NULL);

  muxer = gst_element_factory_make ("flvmux","muxer");
  g_object_set(muxer,"name","mux","streamable",true,NULL);

  encoder = gst_element_factory_make ("x264enc","encoder");
  // g_object_set(encoder, "bitrate", 4000000, NULL);
  // g_object_set(encoder, "iframeinterval", 30, NULL);

  convert = gst_element_factory_make ("videoconvert", "video-converter");

  if ( !pgie || !sink || !muxer || !convert || !encoder)
  {
    g_printerr ("pgie or sink or muxer or convert or encoder element could not be created. Exiting.\n");
    return -1;
  }
  ...
  // g_object_set(G_OBJECT(source), "sensor-id", 0, "framerate", 30/1, NULL);
  ...
  if (!cap_filter1) {
      g_printerr ("Could not create 'cap_filter1'.\n");
      return -1;
  }
  ...
  gst_bin_add_many (GST_BIN (pipeline), convert,cap_filter1,nvvidconv1, nvvidconv2, cap_filter, NULL);
  if (!gst_element_link_many (source, convert,cap_filter1,nvvidconv1, nvvidconv2, cap_filter, NULL)) 
  {
  ...
  if (!srcpad) {
    g_printerr ("cap_filter request src pad failed. Exiting.\n");
    return -1;
  }
  ...
  gst_bin_add_many (GST_BIN (pipeline),streammux,pgie, nvvidconv, nvosd, transform, encoder,muxer,sink, NULL);

#ifdef PLATFORM_TEGRA
  if (!gst_element_link_many (streammux , pgie,
      nvvidconv, nvosd, transform,encoder, muxer,sink,NULL)) {
  ...
  // {
  //   g_printerr ("muxer,sink could not be linked \n");
  //   return -1;
  // }
